...
$('#customerCodeOption').change(function() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/customer/search/"+ selectCustomerId,
        contentType: "application/json",
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
...
const getAllEmployeeSendAJAX = () => {
    $.ajax({
        ...
        success: function(data) {
            var dropdown = $('#empCodeOption');
            data.forEach(employee => {
                dropdown.append($('<option></option>').attr('value', employee.employeeCode).text(employee.employeeCode));
            });
        },
        error: function(xhr, status, error) {
            alert("Failed");
...
// Select Employee Name and Set Text Field
$('#empCodeOption').change(function() {
    var selectEmployeeId = $('#empCodeOption').val(); // Retrieve the selected employee ID

    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/employee/search/" + selectEmployeeId, // Construct the URL with the selected employee ID
        contentType: "application/json",
        beforeSend: function(xhr) {
        ...
        success: function(data) {
            // On success, set the employee name in the text field
            $('#empNameTxt').val(data.employeeName);
        },
        error: function(xhr, status, error) {
            // On error, alert the user
            alert("Failed to retrieve employee details: " + error);
        }
    });
});
...
const getAllItems = () => {
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/item/allItems",
        contentType: "application/json",
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
...
$('#itemCodeOption').change(function() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/item/selectItemSizesGet/"+ selectItemId,
        contentType: "application/json",
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
...
$('#itemCodeOption').change(function (){
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/item/search/"+ selectItemId,
        contentType: "application/json",
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
...
$('#orderBuyBtn').on('click', ()=>{
    ...
    if (validate(orderId,"Order Id") && validate(date,"Date") && validate(customerCode,"Customer Code") && validate(customerName,"Customer Name") && validate(itemCode,"Item Code") && validate(itemName,"Item Name") && validate(size,"Size") && validate(qty,"Quantity") && validate(unitPrice,"Unit Price") && validate(empId,"Employee Id") && validate(empName,"Employee Name")&& validate(payMethod,"Select Payment Method")){
        ...
        $.ajax({
            type: "POST",
            url: "http://localhost:9090/shoes/order/save",
            contentType: "application/json",
            data: orderData,
            beforeSend: function(xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
...
function checkHaveItemQtyAndSetNotify(){
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/stock/sendInfoItemQty",
        contentType: "application/json",
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
...
function generateNewOrderId(){
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/order/newOrderId",
        contentType: "application/json",
        data: false,
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
...
function clearCart() {
    ...
    // Clear table rows
    $("#cart_Table").empty();  // This removes all existing table rows
