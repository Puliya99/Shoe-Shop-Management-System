...
$('#itemBuyBtn').on('click', ()=>{
    ...
    if(validate(itemId,"Item Id") && validate(itemDesc,"Item Decription") && validate(itemType,"Item Type") && validate(occasionType,"Occasion Type") && validate(verities,"Verities") && validate(category,"Category") && validate(size,"Size") && validate(qty,"Quantity") && validate(buyPrice,"Buy Price") && validate(salePrice,"Sale Price") && validate(supId,"Supplier Id") && validate(expectedProfit,"Expected Profit") ){
        ...
        const sendAJAX = (itemDetails,jwtToken) => {
            $.ajax({
                type: "POST",
                url : "http://localhost:9090/shoes/item/save/" + supId + "/" + size + "/" + qty,
                processData: false,
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                const sendAJAX = (itemDetails,jwtToken) => {
                    $.ajax({
                        type: "PUT",
                        url : "http://localhost:9090/shoes/item/update/" + itemId + "/" + supId,
                        processData: false,
                        mimeType: "multipart/form-data",
                        contentType: false,
                        data: form,
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                const sendAJAX = (jwtToken) => {
                    $.ajax({
                        type: "DELETE",
                        url: "http://localhost:9090/shoes/item/delete/"+ itemId,
                        beforeSend: function(xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + jwtToken);
                        },
                        success: function(data) {
...
$('#itemSearchBtn').on('click', ()=>{
    ...
    if (validate(itemId,"Search Item Id")){
        $.ajax({
            type: "GET",
            url: "http://localhost:9090/shoes/item/search/"+itemId,
            contentType: "application/json",
            beforeSend: function(xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + jwtToken);
            },
...
function handleChange() {
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/item/lastId",
        contentType: "application/json",
        beforeSend: function (xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
...
function clearTextFields() {
    ...
    $("#salePriceTxt").val("");
    $("#buyPriceTxt").val("");
    $("#supplierCodeOption").val("");
    $("#expectedProfitTxt").val("");
}
