...
$('#employeeSaveBtn').on('click', ()=>{
    ...
    var addr4 = $('#address4Txt').val();
    var addr5 = $('#address5Txt').val();
    ...
    if (validate(empCode, "Employee Code") && validate(empName, "Employee Name") && validate(gender, "Gender") && validate(status, "Status") && validate(designation, "Designation") && validate(role, "Role") && validate(dob, "Date of Birth") && validate(jd, "Join Date") && validate(branch, "Branch") && validate(addr1, "Address Line 1") && validate(addr2, "Address Line 2") && validate(addr3, "Address Line 3") && validate(addr4, "Address Line 4") && validate(addr5, "Address Line 5") && validate(contact, "Contact Number") && validate(mail, "Email") && validate(gn, "Guardian Name") && validate(emrCont, "Emergency Contact Number") && validate(image,"Employee Image")) {
        ...
        form.append("address4", addr4);
        form.append("address5", addr5);
        ...
        const sendAJAX = (empDetails,jwtToken) => {
            $.ajax({
                type: "POST",
                url : "http://localhost:9090/shoes/employee/save",
                processData: false,
...
$('#employeeSearchBtn').on('click', ()=>{
    ...
    if (validate(searchEmpId,"Search Employee Id")){
        ...
        const sendAJAX = () => {
            $.ajax({
                type: "GET",
                url: "http://localhost:9090/shoes/employee/search/" + searchEmpId,
                ...
                success: function (data) {
                    ...
                    if (!data) {
                        Swal.fire({
                            title: "Sorry This Id Have No Employee !",
                            icon: "info"
                        });
                    }else {
                        ...
                        $('#address4Txt').val(data.address4);
                        $('#address5Txt').val(data.address5);
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                const sendAJAX = (empDetails,jwtToken) => {
                    $.ajax({
                        method: "PUT",
                        url : "http://localhost:9090/shoes/employee/update/"+ updateEmpCode,
                        processData: false,
                        mimeType: "multipart/form-data",
                        contentType: false,
                        data: form,
    ...
    if (validate(deleteEmpId,"Delete Employee Id") && validate(deleteEmpMail,"Delete Employee Mail")){
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "http://localhost:9090/shoes/employee/delete/"+ deleteEmpMail +"/"+deleteEmpId,
                    contentType: "application/json",
                    beforeSend: function(xhr) {
                        xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
                    },
...
const getAllEmployees = () => {
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/employee/allEmployees",
        contentType: "application/json",
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
        success: function(data) {
            data.forEach(employees => {
                var newRow = "<tr><th scope='row'>" + employees.employeeCode + "</th><td>" + employees.employeeName + "</td><td>" + employees.gender + "</td><td>" + employees.status + "</td><td>" + employees.designation   + "</td><td>" + employees.role + "</td><td>" + employees.birthDay  +  "</td><td>" + employees.joinDate +  "</td><td>" + employees.attachedBranch +  "</td><td>" + employees.address1 +  "</td><td>" + employees.address2 +  "</td><td>" + employees.address3 +  "</td><td>" + employees.address4 +  "</td><td>" + employees.address5 + "</td><td>"  + employees.contact + "</td><td>"  + employees.email   + "</td><td>"  + employees.guardianName + "</td><td>"  + employees.emergencyContact + "</td></tr>";
...
$("#employee_Table").on("click", "tr", function() {
    ...
    // Assuming employeePic is part of the data attributes in the row, like <tr data-employee-pic="url">
    let employeePic = $(this).data('employee-pic');
    imageElement.src = employeePic;
    ...
    $("#address4Txt").val(data.eq(11).text());
    $("#address5Txt").val(data.eq(12).text());
    $("#contactTxt").val(data.eq(13).text());
    $("#mailTxt").val(data.eq(14).text());
    $("#guardiaNameTxt").val(data.eq(15).text());
    $("#emgContactTxt").val(data.eq(16).text());
});
