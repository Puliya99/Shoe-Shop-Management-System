export function employeeWiseGetAllOrder(empMail) {
    const sendAJAX = () => {
        $.ajax({
            type: "GET",
            url: "http://localhost:9090/shoes/order/employeeSaleOrders/"+empMail,
...
export function getAllOrder() {
    const sendAJAX = () => {
        $.ajax({
            type: "GET",
            url: "http://localhost:9090/shoes/order/getAllOrders",
            beforeSend: function (xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
            },
            success: function (data) {
...
$('#orderBranchOption').change(function() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:9090/shoes/order/branchWiseOrderDetailsGet/"+selectBranch,
        beforeSend: function(xhr) {
            xhr.setRequestHeader("Authorization", "Bearer " + localStorage.getItem("jwtToken"));
        },
        success: function(data) {
